...
public class ConnectionPool {
	...
	private BlockingQueue<Connection> inPoolConnectionQueue;
	...
	private ConnectionPool() {
		...
		this.password = dbResourceManager.getValue(DBParameter.BD_PASSWORD);
		try {
	...
	private void initPoolData() throws ConnectionPoolException {
		...
		try {
			...
			inPoolConnectionQueue = new ArrayBlockingQueue<Connection>(poolSize);
			givenAwayConnectionQueue = new ArrayBlockingQueue<Connection>(poolSize);
			for(int i = 0; i < poolSize; i ++) {
				Connection connection = DriverManager.getConnection(url, user, password);
				PooledConnection pooledConnection = new PooledConnection(connection);
				inPoolConnectionQueue.add(pooledConnection);
			}
	...
	private void closeConnectrionQueue(BlockingQueue<Connection> queue) throws SQLException {
	...
	public Connection takeConnection() throws ConnectionPoolException {
		...
		try {
			connection = inPoolConnectionQueue.take();
	...
	private class PooledConnection implements Connection {
		...
		public void close() throws SQLException{
			...
			if (!inPoolConnectionQueue.offer(this)) {
